Feature: Timezone
In order to validate timezone generation
As an API client
I want to send different timezone requests and assure the image is generated

    Scenario Outline: Send timezone requests
    ...
    Scenario: Timezone should not have any black borders
        Given I have a timezone request with following parameters:
          | Text    | Width | Height | Difficulty |
          | Bonjour |       |        | Easy       |
        When I send the request to the Create endpoint of the TimezoneController
